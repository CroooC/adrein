...
function setup() {
    ctx.clearRect(0, 0, canvas.width, canvas.height); // Clear the canvas for new game
    drawGallows();
...
function drawGallows() {
    ...
    ctx.moveTo(10, 190); // Base of the gallows
    ctx.lineTo(110, 190); // Base of the gallows
    ctx.moveTo(40, 190); // Vertical post
    ctx.lineTo(40, 10); // Draw the vertical line
    ctx.lineTo(140, 10); // Draw the horizontal line at the top
    ctx.moveTo(140, 10); // Draw the noose
    ctx.lineTo(140, 30);
...
function drawHangman() {
    ...
    // Adjusted drawing positions to avoid overlap with gallows
    const baseX = 140; // Base X position for hangman
    const baseY = 30;  // Base Y position for the noose

    switch (lives) {
        case 6: // Head
            ctx.arc(baseX, baseY + 20, 20, 0, Math.PI * 2, true); // Position adjusted
            ctx.stroke();
            break;
        case 5: // Body
            ctx.moveTo(baseX, baseY + 40);
            ctx.lineTo(baseX, baseY + 100);
            ctx.stroke();
            break;
        case 4: // Left Arm
        ...
        case 1: // Right Leg
            ...
            ctx.stroke();
            break;
        case 0: // Hangman Complete
            ctx.moveTo(baseX - 25, baseY - 5); // Cross out head
            ctx.lineTo(baseX + 25, baseY + 45);
            ctx.stroke();
            break;
    }
}
...
function checkGameOver() {
    ...
    resetBtn.classList.remove('hide'); // Show the popup
    resetBtn.classList.add('show');

    resetBtn.addEventListener('click', () => {
        resetGame(); // Restart the game and hide the popup
        popupContainer.classList.remove('show');
        popupContainer.classList.add('hide');
        resetBtn.classList.remove('show');
        resetBtn.classList.add('hide'); // Hide reset button after restart
    });

    if (lives === 0) {
        popupMessage.textContent = 'You died, sorry. The word was ' + word;
        popupContainer.classList.remove('hide'); // Show the popup
        popupContainer.classList.add('show');
        popupContainer.style.color = 'red'; // Red color for losing
    } else if ([...wordLetters].every(letter => usedLetters.has(letter))) {
        popupMessage.textContent = 'YAY! You guessed the word ' + word + '!!';
        popupContainer.classList.remove('hide'); // Show the popup
        popupContainer.classList.add('show');
        popupContainer.style.color = 'green'; // Green color for winning
    }
}
...
function resetGame() {
    ...
    setTimeout(() => {
        setup();              // Reset word and start a new game
    }, 100);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    guessButton.addEventListener('click', () => {
        ...
        if (wordLetters.has(guess)) {
            ...
            lives--;
            drawHangman(); // Call drawHangman() after a wrong guess
        ...
        // Check game over status after each guess
        if (lives === 0 || [...wordLetters].every(letter => usedLetters.has(letter))) {
            checkGameOver();
        }
    });
});
